...
export default function CheckoutForm() {
  ...
  if (!items.length) return <div className="empty-page"><h1>Your bag is empty.</h1><p>Add a product before continuing.</p><Link href="/#configure" className="button primary">Configure iPhone 17 Pro Max</Link></div>;
  ...
  </form><aside className="checkout-summary"><h2>Your order</h2>{items.map(item => <div className="checkout-item" key={item.id}><img className="checkout-thumb" src={iphone17ProMax.images.finishes} alt="iPhone 17 Pro Max"/><span>{item.quantity}</span><p><b>{item.name}</b><small>{item.storage}</small></p><strong>{formatPrice(item.unitPrice * item.quantity)}</strong></div>)}<dl><div><dt>Subtotal</dt><dd>{formatPrice(subtotal)}</dd></div><div><dt>Shipping</dt><dd>{delivery === "express" ? "$9" : "Free"}</dd></div></dl><div className="summary-total"><span>Total</span><b>{formatPrice(subtotal + (delivery === "express" ? 9 : 0))}</b></div></aside></div></div>;
}
