...
if (missingFiles.length || missingValues.length || missingStories.length || foundConceptCode.length || removedConceptAssets.length || missingCartBehaviors.length) {
  console.error({ missingFiles, missingValues, missingStories, foundConceptCode, removedConceptAssets, missingCartBehaviors });
  process.exit(1);
}

console.log(`Validated ${routes.length} static routes, one product source, ${requiredStories.length} official-image product stories, no conceptual device artwork, and persistent cart behavior.`);
